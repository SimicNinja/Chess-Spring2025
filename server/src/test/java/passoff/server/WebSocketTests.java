...
public class WebSocketTests
{
    ...
    {
        connectToGame(new WebsocketUser("didn't register", "badAuth"), gameID, false, Set.of(), Set.of(),
                "connect " + "with bad auth");
    }
    ...
    public void makeMoveBadAuthtoken()
    {
        ...
        makeMove(new WebsocketUser(white.username(), "badAuth"), gameID, move, false, false, Set.of(black, observer),
                Set.of(), "move made with bad authtoken");
    }
                          ...
                          String description)
    {
        ...
        {
            assertCommandMessages(actualMessages, expectSuccess, sender, types(LOAD_GAME), inGame, types(LOAD_GAME,
                    NOTIFICATION), otherClients, description);
        }
    }
                                ...
                                List<TestMessage> messages, String description)
    {
        Assertions.assertEquals(expectedTypes.length, messages.size(), ("For command '%s' user '%s' expected %d " +
                "messages with types %s, got %d: %s").formatted(description, username, expectedTypes.length,
                Arrays.toString(expectedTypes), messages.size(), messages));
    ...
    {
        Assertions.assertEquals(ServerMessage.ServerMessageType.LOAD_GAME, message.getServerMessageType(), ("Message "
                + "for %s was not a LOAD_GAME message: %s").formatted(username, message));
    ...
    private void assertNotification(String username, TestMessage message)
    {
        ...
        Assertions.assertNotNull(message.getMessage(),
                ("%s's NOTIFICATION message did not contain a message (Make " + "sure it's specifically called " +
                        "'message')").formatted(username));
        Assertions.assertNull(message.getGame(), "%s's NOTIFICATION message contained a game: %s".formatted(username,
                message.getGame()));
        Assertions.assertNull(message.getErrorMessage(),
    ...
    {
        Assertions.assertEquals(ServerMessage.ServerMessageType.ERROR, message.getServerMessageType(), ("Message for "
                + "%s was not an ERROR message: %s").formatted(username, message));
        ...
        Assertions.assertNull(message.getGame(), "%s's ERROR message contained a game: %s".formatted(username,
                message.getGame()));
        Assertions.assertNull(message.getMessage(),
