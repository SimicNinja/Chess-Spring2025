...
public class WebSocketTests
{
	...
	public void multipleConcurrentGames()
	{
		...
		connectToGame(observer2, otherGameID, true, Set.of(white2, black2), Set.of(white, black, observer), "connect " + "3 to other " +
				"game");
	...
	private void assertHttpOk(TestResult result, String context)
	{
		Assertions.assertEquals(200, serverFacade.getStatusCode(),
				String.format("HTTP Status code was not 200 for " + "%s, was %d. Message: %s", context, serverFacade.getStatusCode(),
						result.getMessage()));
	}
							   ...
							   Set<WebsocketUser> otherClients, String description)
	{
		...
		Map<String, List<TestMessage>> actualMessages = environment.exchange(sender.username(), connectCommand, numExpectedMessages,
				waitTime);
						  ...
						  Set<WebsocketUser> inGame, Set<WebsocketUser> otherClients, String description)
	{
		...
		Map<String, List<TestMessage>> actualMessages = environment.exchange(sender.username(), moveCommand, numExpectedMessages,
				waitTime);
		...
		if(extraNotification && actualMessages.get(sender.username()).size() > 1)
		{
			assertCommandMessages(actualMessages, expectSuccess, sender, types(LOAD_GAME, NOTIFICATION), inGame,
					types(LOAD_GAME, NOTIFICATION, NOTIFICATION), otherClients, description);
		}
		else
		{
			assertCommandMessages(actualMessages, expectSuccess, sender, types(LOAD_GAME), inGame, types(LOAD_GAME, NOTIFICATION),
						...
						String description)
	{
		...
		Map<String, Integer> numExpectedMessages = expectedMessages(sender, 1, inGame, (expectSuccess ? 1 : 0), otherClients);
		Map<String, List<TestMessage>> actualMessages = environment.exchange(sender.username(), resignCommand, numExpectedMessages,
				waitTime);

		assertCommandMessages(actualMessages, expectSuccess, sender, types(NOTIFICATION), inGame, types(NOTIFICATION), otherClients,
				description);
	}
	...
	private void leave(WebsocketUser sender, int gameID, Set<WebsocketUser> inGame, Set<WebsocketUser> otherClients, String description)
	{
		...
		Map<String, List<TestMessage>> actualMessages = environment.exchange(sender.username(), leaveCommand, numExpectedMessages,
				waitTime);

		assertCommandMessages(actualMessages, true, sender, types(), inGame, types(NOTIFICATION), otherClients, description);
	}

	private Map<String, Integer> expectedMessages(WebsocketUser sender, int senderExpected, Set<WebsocketUser> inGame, int inGameExpected,
												  Set<WebsocketUser> otherClients)
	{
	...
	private void assertMessages(String username, ServerMessage.ServerMessageType[] expectedTypes, List<TestMessage> messages,
								String description)
	{
		Assertions.assertEquals(expectedTypes.length, messages.size(),
				("For command '%s' user '%s' expected %d " + "messages with types %s, got %d: %s").formatted(description, username,
						expectedTypes.length, Arrays.toString(expectedTypes), messages.size(), messages));
